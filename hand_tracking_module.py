...
class HandDetector():
    ...
    def findPosition(self,img,handNo=0,draw=True):     
        ...
        if self.result.multi_hand_landmarks:
            myHand = self.result.multi_hand_landmark[handNo]
            for id, lm in enumerate(myHand.landmark):
                # print(id,"\n", lm)
                h, w, c = img.shape #height ,width , channels
                cx, cy =  int(lm.x*w), int(lm.y*h)  #centre points
                # print(id,cx,cy)
                lmList = list.append([id, cx, cy])
...
def main():
    ...
    while True:
        ...
        if len(lmList)!=0:
            print(lmList[4]) # 4 was tip of the thumb and only it gives the position of specific list
